...
description: Write a literature review based on papers in a workspace. Covers topic
  organization, narrative structure, gap identification, and BibTeX export. Use when
  the user wants to draft a literature review, survey a research area, or summarize
...
- **必读文档**：
  ...
  - 项目记忆中的学术写作指南文件（`academic_writing_guidelines.md`、`academic_writing_rich_elements.md`、`chinese_latex_typesetting.md`）
- **硬性要求**：
...
4. **自动深度阅读（强制提示词模板）**：
   ...
   - 将值得保留的发现自动追加到各论文的 `notes.md`。追加方式可通过 Python API：`python -c "from scholaraio.loader import append_notes; append_notes('data/papers/<dir>', '## YYYY-MM-DD | <workspace> | literature-review-fast\n...')"`，或由主 agent 直接读写文件实现
...
分析完成后，将值得跨会话保留的关键发现追加到论文的 `notes.md`。追加方式：通过 Python API `python -c "from scholaraio.loader import append_notes; append_notes('data/papers/<dir>', '## YYYY-MM-DD | <workspace> | literature-review\n...')"`，或由主 agent 直接读写文件实现。格式：`## YYYY-MM-DD | <workspace> | literature-review`，内容包括方法特点、核心贡献、与其他论文的关键对比。
...
3. 将图片从 `data/papers/<dir>/images/` 复制到 `workspace/<name>/images/`（或 `published/<dir>/images/`）
...
6. **插图审计脚本（必须执行）**：
   ...
   - **不要删除**工作目录中的图片文件，仅记录哪些图片未被引用；由 `publish` 技能在归档时过滤
7. **引用闭合检查**：
...
10. 打开 PDF 进行最终视觉检查：
    ...
    - 参考文献格式统一
    - 摘要与结论覆盖了全部章节，无遗漏
11. **若产物位于 `published/` 目录，同步更新 `metadata.json`**：修正页数、关键词和 note，使其与实际 PDF 一致
