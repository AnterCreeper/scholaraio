...
```bash
# 下载PDF → inbox/
curl -o data/inbox/xxx.pdf <URL>

# 批量入库
scholaraio pipeline ingest

# 验证入库
ls data/papers/ | grep <作者>
...
0. **源论文插图**（正文章节完成后补充插入）
   ...
   \begin{figure}[htbp]
   \centering
   \includegraphics[width=0.8\textwidth]{../../data/papers/作者-年份-标题/images/图名.jpg}
...
2. **提取图片**
   ```bash
   # 查看源论文目录
   ls data/papers/<作者>-<年份>-<标题>/images/
   
   # 复制到综述images目录（或直接用相对路径引用）
   mkdir -p workspace/<综述目录>/images/
   cp data/papers/<作者>-<年份>-<标题>/images/figX.jpg \
